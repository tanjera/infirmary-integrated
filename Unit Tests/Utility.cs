    ...
    public class Utility {
        ...
        public void IsNewerVersion () {
            ...
            for (int i = 0; i < t1_p1.Length && i < t1_p2.Length; i++) {
                ...
                Assert.IsTrue (II.Utility.IsNewerVersion (t1_p1 [i], t1_p2 [i]));
                // Test newer -> older version numbers
                Assert.IsFalse (II.Utility.IsNewerVersion (t1_p2 [i], t1_p1 [i]));
                // Test version numbers against same
                Assert.IsFalse (II.Utility.IsNewerVersion (t1_p1 [i], t1_p1 [i]));
            }
        }
    }
}